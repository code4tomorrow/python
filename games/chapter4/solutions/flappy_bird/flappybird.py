...
def load_sprites(spritesheet, DIMw, DIMh, offset):
    ...
    for i in range(
        DIMw * DIMh - offset
    ):  # offset is if there are extra blank frames
    ...
    if KEY_UP == True and birdrect[1] > 10:
        if acceleration >= 0:
            ...
            if HIT_COIN == True:
                HIT_COIN = False
                score += 1
        else:  # if acceleration is less than 0, go back to falling
            if birdrect[1] < 580:
                birdrect[1] += 4
    else:
        if birdrect[1] < 580:
...
def losepage(screen):
    pygame.draw.rect(
        screen, PINK, (0, 0, screen.get_width(), screen.get_height())
    )
...
def getNewState(but, curState, x, y):
    if but == 1 and curState == MENUSTATE and 150 < x < 650 and 100 < y < 200:
        # during menu state if button clicked, gamestate begins
        return GAMESTATE
    elif (
        but == 1 and curState == MENUSTATE and 150 < x < 650 and 300 < y < 400
    ):
        return QUITSTATE
    elif (
        but == 1 and curState == LOSESTATE and 150 < x < 650 and 100 < y < 200
    ):
        print("you lost")
        return GAMESTATE
    elif (
        but == 1 and curState == LOSESTATE and 150 < x < 650 and 300 < y < 400
    ):
        return QUITSTATE
    else:  # will happen for clicks outside of boxes on the "menu"
        print("check test")
        print("button is", but)
        return curState
        # if this isn't done, then a click outside of box will cause an
        # effective crash
    ...
    if state == GAMESTATE:
        ...
        for element in rectList:
            if collision(birdrect, element) == True:
                state = LOSESTATE
        if (
            collision(birdrect, drawCoin(topGap)) == True
            and drawCoin(topGap)[0] == 145
        ):
            HIT_COIN = True
