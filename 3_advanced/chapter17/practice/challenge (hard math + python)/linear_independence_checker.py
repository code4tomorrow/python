# In linear algebra, a vector (list) of  n vectors (lists) each containing n
# integers are considered linearly independent if we solve the following equation
# a * x   +  a * x   +  a * x  +  a * x  = 0
#  1   1      2   2      3   3     n   n
# for  where they can be any real constant, and we find that all of them equal to
# 0 being the only possible solution for this system of equations (this is called 
# the trivial solution). Write a method which takes a vector of n vectors each of
# size n and determine if they are linearly independent. Return true if linearly 
