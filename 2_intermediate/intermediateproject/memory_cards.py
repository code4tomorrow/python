...
class memcards:
    ...
    def play(self):
        instructions = "The Rows and Columns start at 0, not 1; # means unflipped; _ means correct"
        # I used a set, but you could also have used a list or a dictionary so long as
        # you checked if elements were already in the list before adding them
        flipped = set()
        print(instructions)
        while len(self.memorder) > len(flipped):
            ...
            if item1 in self.dictionary and self.dictionary[item1] == item2:
                print("Correct!")
                flipped.add(self.flip(row1, column1, "correct"))
                flipped.add(self.flip(row2, column2, "correct"))
            elif item2 in self.dictionary and self.dictionary[item2] == item1:
                print("Correct!")
                flipped.add(self.flip(row1, column1, "correct"))
                flipped.add(self.flip(row2, column2, "correct"))
            else:
