...
class memcards(tk.Frame):
    ...
    def createlabel(self):
        self.label = tk.Label(
            self,
            text="The Rows and Columns start at 0, not 1; #"
            + " means unflipped; _ means correct",
        )
        self.label.grid(
            row=len(self.memorder) // len(self.currmap[0]),
            column=0,
            rowspan=2,
            columnspan=len(self.currmap[0]),
            sticky=tk.W + tk.S,
        )
    ...
    def definemap(self):
        ...
        for x in range(height):
            for i in range(width):
                themap[x][i] = tk.Button(
                    self, text="#", command=partial(self.flip, x, i)
                )
    ...
    def flip(self, row, column):
        ...
        self.current.append(
            [
                self.memorder[(row * len(self.currmap[0])) + column],
                [row, column],
            ]
        )
    ...
    def unflip(self, correct: bool):
        if correct:
            ...
            self.currmap[self.current[0][1][0]][self.current[0][1][1]][
                "text"
            ] = "__"
            self.currmap[self.current[1][1][0]][self.current[1][1][1]][
                "text"
            ] = "__"
        ...
        try:
            while 1:
                ...
                if len(self.current) == 2:
                    if (
                        self.current[0][0] in self.dictionary
                        and self.dictionary[self.current[0][0]]
                        == self.current[1][0]
                    ):
                        time.sleep(0.5)
                        self.unflip(True)
                    elif (
                        self.current[1][0] in self.dictionary
                        and self.dictionary[self.current[1][0]]
                        == self.current[0][0]
                    ):
                        time.sleep(0.5)
                        self.unflip(True)
